...
```json
{
  ...
  "lines": [
    ...
    },
    {
      ...
      "type": "dialogue",
      "character": "사쿠라",
      "characterImage": "sakura_smile.png",
      "characterPosition": "center-left",
      "background": "school_entrance.svg",
      "text": "안녕! 나는 사쿠라야. 반가워!",
      "nextScriptId": "meet_yuki"
...
### 1. 파일 및 ID 명명 규칙
- **씬 파일명**: `01_Scene_Name.json`, `02_Event_Name.json` (숫자 접두사 + 설명)
- **엔딩 파일**: `Ending_CharacterName.json`, `Ending_True.json`
- **캐릭터 이미지**: `캐릭터ID_표정명.png` (예: `seolah_smile.png`, `sakura_panic.png`)
...
### 3. 챕터 설계 가이드
- **Chapter 1**: 프로로그, 캐릭터 소개, 세계관 설정 (각 씬당 10-20라인 권장)
- **Chapter 2-4**: 메인 스토리, 선택지에 따른 분기 (각 씬당 15-30라인 권장)
- **Chapter 5**: 엔딩 (각 엔딩당 5-10라인 권장)
...
### 씬 파일 수
- **Chapter 1**: 3-5개 씬 파일 (프로로그, 짧고 임팩트 있게)
- **Chapter 2-4**: 각 5-10개 씬 파일 (메인 스토리, 충분한 분기)
- **Chapter 5**: 3-6개 엔딩 파일

### 씬당 라인 수 (중요!)
- **프로로그 씬**: 10-15개 라인 (세계관 설명, 분위기 조성)
- **일반 대화 씬**: 15-25개 라인 (캐릭터 상호작용, 스토리 전개)
- **중요 이벤트 씬**: 20-30개 라인 (전투, 갈등, 클라이맥스)
- **선택지 씬**: 10-15개 라인 (상황 설명 + 선택지 + 결과)
- **엔딩 씬**: 5-10개 라인 (감동적인 마무리)
...
**권장**: 15-25개 라인 (풍부한 스토리텔링, **이것을 목표로!**)
**최적**: 25-30개 라인 (중요 이벤트, 클라이맥스)
